...
template "#{node['weblogic-domain']['response_file_dir']}/configure_#{node['weblogic-domain']['domain_name']}_nodemanager.py" do
  source "configure_nodemanager.py.erb"
...
execute "wlst.sh configure_#{node['weblogic-domain']['domain_name']}_nodemanager.py" do
  environment "CONFIG_JVM_ARGS" => "-Djava.security.egd=file:/dev/./urandom"
  command <<-EOH 
    . #{node['weblogic-domain']['wls_home']}/server/bin/setWLSEnv.sh
    #{node['weblogic-domain']['oracle_common']}/common/bin/wlst.sh #{node['weblogic-domain']['response_file_dir']}/configure_#{node['weblogic-domain']['domain_name']}_nodemanager.py
  EOH
